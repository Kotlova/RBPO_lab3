...
namespace Math {
	...
	export Complex pow(const Complex& base, Complex exp) {
		double first = log(base.Mod());
		double second = atan2(base.Im(), base.Re());
		Complex tempExp = exp * Complex(first, second);
		return Complex(std::pow(3.14, first),0) * Complex::FromExponentialForm(1, second);
	}
	...
	export Rational pow(const Rational& obj, Rational exponent) {
		double exp = (double)exponent.Nominator() / exponent.Denominator();
		int tempNominator = (int)(std::pow(obj.Nominator(), exp)*10000);
		int tempDenominator = (int)(std::pow(obj.Denominator(), exp) * 10000);
	...
	export Complex f(const Complex& u) {
		Complex a = Complex(0.0, 1);
		return Math::pow(u,3) + (1 + 2*u) * u * u + (1 - 2*u) * Math::pow(u,-5*a);
	}
	...
	export double f(double u) {
		double a = 0.5;
		return std::pow(u, 3) + (1 + 2 * u) * u * u + (1 - 2 * u) * std::pow(u, -5 * a);
	}
}